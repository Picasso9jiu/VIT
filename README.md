...
|------|------|-----|---------|------|------|
| FP32 基准 | WSL2 | 65.4 | 1.00× | 基线 | — |
| BF16 | WSL2 | 191.5 | 2.93× | 高性能 | Ampere Tensor Core，原生支持 |
...
| **我们的 W8A8 + 自研 kernel** | **WSL2** | **166.3** | **2.54×** | **显著加速** | 自定义 Triton kernel，使用我们的 Δ/zp |
| **我们的 W4A4 + 自研 kernel** | **WSL2** | **158.7** | **2.43×** | **显著加速** | 同上，4-bit 权重存储 |
...
- **Windows 平台**：没有任何方案能对 ViT 量化模型产生实质性 INT4/INT8 加速。最佳实践是直接用 BF16（2.9×）
- **WSL2 + Triton**：通过自定义 kernel 实现了 2.5× 加速，且使用的是我们 PTQ 学出的原始量化参数
- **平台差距根源**：Triton 只在 Linux 可用；cuBLAS/TensorRT 的 INT8 API 只接受对称量化；Windows 上缺少可编程 GPU kernel 框架

### 2.7 W8A8 与 W4A4 速度相近的原因

**发现过程：** 在首次跑通 `ex_benchmark.py` 后，我们观察到 W8A8（166.3 FPS）和 W4A4（158.7 FPS）的速度几乎相同，差距仅 5%。这与直觉——INT4 权重内存带宽为 INT8 的一半、应当更快——明显矛盾。我们怀疑是 kernel 实现存在性能缺陷，随即进行了系统排查。
...
> *"At equivalent inference speed (2.54× for W8A8 vs 2.43× for W4A4), our W4A4 model halves weight storage compared to W8A8, and achieves 8× model size reduction versus FP32——critical for edge deployment where storage and memory are primary constraints."*
...
|------|----------|-----|---------|---------|---------|---------|
| FP32 基准 | PyTorch (WSL2) | 65.4 | 1.00× | — | FP32 | 基线 |
| BF16 | PyTorch (WSL2) | 191.5 | 2.93× | 无损 | FP16/BF16 | Ampere Tensor Core 原生加速 |
...
| **我们的 W8A8** | **Triton (WSL2)** | **166.3** | **2.54×** | **Top-1 100%** | **INT8** | **自研 kernel，我们的 Δ/zp** |
| **我们的 W4A4** | **Triton (WSL2)** | **158.7** | **2.43×** | **Top-5 100%** | **INT4** | **自研 kernel，我们的 Δ/zp** |
...
> **核心结论**：我们的 W8A8/W4A4 + 自研 Triton kernel 是唯一一个同时满足"使用我们 PTQ 学出的量化参数"和"实现真实硬件加速"的方案。BF16 更快（2.93×）但无压缩；通用 INT8/INT4 库有加速但不认我们的参数。我们的方案在精度-速度-存储三者间取得了最佳平衡。
...
直觉上 INT4 应该比 INT8 快（4× 内存带宽节省），但实验证明在 RTX 3050 + ViT-Base 场景下两者速度几乎一样（2.54× vs 2.43×）。原因在于 ViT 的矩阵乘规模（最大 768→3072）使得计算成为瓶颈而非访存——RTX 3050 的 9 TFLOPS 算力先于 192 GB/s 显存带宽达到饱和。这提醒我们：**量化加速是一个系统性问题，必须综合考虑模型规模、硬件算力/带宽比、矩阵形状等因素**，不能仅凭理论比特宽下结论。
