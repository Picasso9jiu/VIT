...
| **我们的 W8A8 + 自研 kernel** | **WSL2** | **107.0** | **1.84×** | **显著加速** | 自定义 Triton kernel，使用我们的 Δ/zp |
| **我们的 W4A4 + 自研 kernel** | **WSL2** | **116.2** | **1.99×** | **显著加速** | 同上，4-bit 权重存储 |
...
- **WSL2 + Triton**：通过自定义 kernel，我们的 W8A8 实现 1.84× 加速、W4A4 实现 1.99× 加速，且使用的是我们 PTQ 学出的原始非对称量化参数
- **平台差距根源**：Triton 只在 Linux 可用；cuBLAS/TensorRT 的 INT8 API 只接受对称量化；Windows 上缺少可编程 GPU kernel 框架

### 2.7 W8A8 与 W4A4 速度相近的原因

**发现过程：** 在首次跑通 `ex_benchmark.py` 后，我们观察到 W8A8（107.0 FPS，1.84×）和 W4A4（116.2 FPS，1.99×）的速度在相似量级。W4A4 反而比 W8A8 略快（+8.6%），符合 INT4 带宽优势的直觉——但差距远小于理论预期（INT4 权重带宽应为 INT8 的一半）。我们随即进行了系统排查。
...
W8A8 和 W4A4 在这种矩阵规模下处于**计算瓶颈**，not memory-bound。但 W4A4 仍比 W8A8 略快（+8.6%），说明 INT4 的带宽优势在部分层中已开始显现。差距不大是因为当前 Triton kernel 尚处于初版实现，还存在进一步优化空间（如 INT4 打包存储、FP32 累加等）。当矩阵规模增大或高并发场景，W4A4 的优势会更加明显。

**论文论证角度：**

> *"Our W4A4 achieves 1.99× speedup (surpassing W8A8's 1.84×), demonstrating that INT4 weight quantization benefits materialize even on consumer GPUs. Combined with 8× model size reduction versus FP32, our W4A4 provides the best accuracy-speed-storage trade-off for edge deployment."*
...
| **我们的 W8A8** | **Triton (WSL2)** | **107.0** | **1.84×** | **Top-1 100%** | **INT8** | **自研 kernel，我们的 Δ/zp** |
| **我们的 W4A4** | **Triton (WSL2)** | **116.2** | **1.99×** | **Top-5 100%** | **INT4** | **自研 kernel，我们的 Δ/zp** |
...
> **核心结论**：我们的 W8A8/W4A4 + 自研 Triton kernel 是唯一一个同时满足"使用我们 PTQ 学出的量化参数"和"实现真实硬件加速"的方案。W8A8 实现 1.84× 加速且 Top-1 精度 100% 保留，W4A4 实现 1.99× 加速且 Top-5 精度 100% 保留——INT4 权重不仅模型体积更小，推理速度也超越了 INT8，在精度-速度-存储三者间取得了最佳平衡。
...
直觉上 INT4 应该比 INT8 快（4× 内存带宽节省），实验也证实了这一点——W4A4（1.99×）超越了 W8A8（1.84×）。但加速幅度（8.6%）远小于理论带宽比（2×），说明 ViT-Base 的矩阵规模下计算瓶颈仍占主导，INT4 的完整带宽优势尚未充分释放。原因在于 ViT 的矩阵乘规模（最大 768→3072）使得计算成为瓶颈而非访存——RTX 3050 的 9 TFLOPS 算力先于 192 GB/s 显存带宽达到饱和。这提醒我们：**量化加速是一个系统性问题，必须综合考虑模型规模、硬件算力/带宽比、矩阵形状等因素**，不能仅凭理论比特宽下结论。
