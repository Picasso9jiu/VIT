...
|------|------|-----|---------|------|------|
| FP32 基准 | WSL2 | 55.6 | 1.00× | 基线 | — |
| FP32 + TF32 | Win | 110.4 | 1.68× | 中等加速 | FP32 matmul 启用 Tensor Core |
| torchao INT8 (通用) | Win | 90.6 | 1.38× | 有加速但有限 | cuBLAS INT8 GEMM，对称量化 |
| torchao INT8 (通用) | WSL2 | 91.3 | 1.37× | 同上 | Triton INT8 kernel，仍为对称量化 |
| torchao INT4 (通用) | Win | — | ≈1.0× | **无效** | Windows 无 Triton，走 Python fallback |
| torchao INT4 (通用) | WSL2 | — | ≈1.0× | **无效** | 同样走解量化→BF16 GEMM，无加速 |
| **我们的 W8A8 + 自研 kernel** | **WSL2** | **119.4** | **2.15×** | **显著加速** | 自定义 Triton kernel，使用我们的 Δ/zp |
| **我们的 W4A4 + 自研 kernel** | **WSL2** | **117.5** | **2.11×** | **显著加速** | 同上，4-bit 权重存储 |
| 我们的 W8A8 伪量化 | WSL2 | 4.8 | 0.07× | 极慢 | Python 量化器破坏 GPU 融合 |
| 我们的 W8A8 ONNX + TRT | Win | 114.0 | 0.91× | 无加速 | round/clamp op 破坏 TensorRT 图优化 |
...
- **Windows 平台**：没有任何方案能对 ViT 量化模型产生实质性 INT4/INT8 加速。无论是通用库（torchao/bnb）还是推理引擎（TensorRT/ORT），均无法直接使用我们的 PTQ 参数或产生加速
- **WSL2 + Triton**：通过自定义 kernel，我们的 W8A8 实现 2.15× 加速、W4A4 实现 2.11× 加速，且使用的是我们 PTQ 学出的原始非对称量化参数
- **平台差距根源**：Triton 只在 Linux 可用；cuBLAS/TensorRT 的 INT8 API 只接受对称量化；Windows 上缺少可编程 GPU kernel 框架

### 2.7 W8A8 与 W4A4 速度相近的原因

**发现过程：** 在首次跑通 `ex_benchmark.py` 后，我们观察到 W8A8（119.4 FPS，2.15×）和 W4A4（117.5 FPS，2.11×）的速度几乎相同，差距仅 1.6%。这与直觉——INT4 权重内存带宽为 INT8 的一半、应当更快——明显矛盾。我们怀疑是 kernel 实现存在性能缺陷，随即进行了系统排查。
...
W8A8 和 W4A4 在这种矩阵规模下是**计算瓶颈**，not memory-bound。INT4 省下的内存带宽没有转化为速度提升，因为计算单元已经满了。只有当矩阵规模增大（如 LLM 的 4096×4096 GEMM）或多请求并发（increased batch size / multi-user serving）使访存成为瓶颈时，INT4 的带宽优势才会体现。

**论文论证角度：**

> *"At equivalent inference speed (2.15× for W8A8 vs 2.11× for W4A4), our W4A4 model halves weight storage compared to W8A8, and achieves 8× model size reduction versus FP32——critical for edge deployment where storage and memory are primary constraints."*
...
|------|----------|-----|---------|---------|---------|---------|
| FP32 基准 | PyTorch (WSL2) | 55.6 | 1.00× | — | FP32 | 基线 |
| FP32 + TF32 | PyTorch (Win) | 110.4 | 1.68× | 无损 | FP32 | 启用 Tensor Core 的 FP32 |
| torchao INT8 (通用) | PyTorch (Win) | 90.6 | 1.38× | — | INT8 | cuBLAS，对称量化，非我们的参数 |
| torchao INT4 (通用) | PyTorch (Win) | — | ≈1.0× | — | INT4 | Windows 无 Triton，无效 |
| **我们的 W8A8** | **Triton (WSL2)** | **119.4** | **2.15×** | **Top-1 100%** | **INT8** | **自研 kernel，我们的 Δ/zp** |
| **我们的 W4A4** | **Triton (WSL2)** | **117.5** | **2.11×** | **Top-5 100%** | **INT4** | **自研 kernel，我们的 Δ/zp** |
| 我们的 W8A8 伪量化 | PyTorch (WSL2) | 4.8 | 0.07× | — | FP32 | Python 量化器，不可部署 |
| 我们的 W8A8 ONNX + TRT | TensorRT (Win) | 114.0 | 0.91× | — | FP32 | round/clamp 破坏图优化 |
| bitsandbytes INT4 | bnb (Win) | 53.0 | 0.92× | — | INT4 | kernel 不适配 ViT |

> **核心结论**：我们的 W8A8/W4A4 + 自研 Triton kernel 是唯一一个同时满足"使用我们 PTQ 学出的量化参数"和"实现真实硬件加速"的方案。W8A8 实现 2.15× 加速且 Top-1 精度 100% 保留，W4A4 实现 2.11× 加速且 Top-5 精度 100% 保留，同时模型体积仅为 FP32 的 1/8，在精度-速度-存储三者间取得了最佳平衡。
...
IaS-ViT 的三阶段流程中最关键的不是量化操作本身，而是 Block/Layer 级的**输出重建**——通过最小化量化前后中间特征的 L2 距离来补偿误差。这启示我们：PTQ 的本质是在精度-效率空间中搜索最优重建点，量化参数只是搜索结果的载体。未来 PTQ 方法的创新应该聚焦于更优的重建目标和更高效的搜索策略，而非简单地堆砌更多校准数据。
...
- 主流推理库（cuBLAS、TensorRT、torchao）全部采用对称量化（zp=0），而论文学出的是非对称量化参数（zp≠0），直接套用会丢弃学到的量化参数

**（3）Consumer GPU 的量化推理瓶颈与直觉相反**

直觉上 INT4 应该比 INT8 快（4× 内存带宽节省），但实验证明在 RTX 3050 + ViT-Base 场景下两者速度几乎一样（2.15× vs 2.11×）。原因在于 ViT 的矩阵乘规模（最大 768→3072）使得计算成为瓶颈而非访存——RTX 3050 的 9 TFLOPS 算力先于 192 GB/s 显存带宽达到饱和。这提醒我们：**量化加速是一个系统性问题，必须综合考虑模型规模、硬件算力/带宽比、矩阵形状等因素**，不能仅凭理论比特宽下结论。

### 4.2 复现过程中的思考

**（1）为什么非对称量化在学术论文中普遍存在，但工业界全用对称量化？**

对称量化（zp=0）可以将量化 GEMM 拆解为 `x @ w_int * scale`，两个操作都可以被 cuBLAS/TensorRT 原生加速。非对称量化多了一个 `zp` 维度，需要额外维护偏置修正项。工业界选择对称量化不是因为它更好，而是因为现有硬件/库的 API 设计就是围绕对称量化展开的。这意味着 PTQ 学术研究提出的非对称量化方案在部署时会遇到兼容性障碍——而解决这个障碍正是我们的自定义 Triton kernel 所做的。

**（2）伪量化 → 真量化的转换本质上是一个"参数映射"问题**

我们的 PTQ 模型有 `delta`、`zero_point`、`weight` 三组参数。转换为真 INT8 推理需要：
...
在找到「WSL2 + Triton + 自定义 kernel」这条可行路径之前，我们系统尝试了 torchao（Windows → Linux）、bitsandbytes、ONNX Runtime、TensorRT 四条路线，积累了完整的对比数据。这些"失败"的尝试不是浪费时间——它们帮助我们精确理解了每套工具链的限制（TensorRT 10.x 废弃 calibrator API、onnxruntime CUDA EP 对 QDQ 优化不足、bitsandbytes 的 kernel 对 ViT 矩阵尺寸不友好等），最终才确定了正确方向。
